...
/**
 * $crt_manage_db_version - holds current database version
...
register_activation_hook(__FILE__, 'crt_manage_site_install');
...
register_activation_hook(__FILE__, 'crt_manage_site_install_data');
...
class Custom_Table_Example_List_Table extends WP_List_Table
{
    ...
    {
        $actions = array(
            'edit' => sprintf('<a href="?page=domains_form&id=%s">%s</a>', $item['id'], __('Edit', 'crt_manage')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['id'], __('Delete', 'crt_manage')),
        );
 ...
 */
function crt_manage_admin_menu()
{
    add_menu_page(__('CRThemes', 'crt_manage'), __('CRThemes', 'crt_manage'), 'activate_plugins', 'crthemes', 'crt_manage_persons_page_handler');
    add_submenu_page('crthemes', __('Add new', 'crt_manage'), __('Add new', 'crt_manage'), 'activate_plugins', 'domains_form', 'crt_manage_persons_form_page_handler');
}

add_action('admin_menu', 'crt_manage_admin_menu');
 ...
 */
function crt_manage_persons_page_handler()
{
    global $wpdb;
    ...
    <div class="wrap">
        ...
        <h2><?php _e('Persons', 'crt_manage')?> <a class="add-new-h2"
 ...
 */
function crt_manage_persons_form_page_handler()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'crtheme_manage_sites'; // do not forget about tables prefix
    ...
    if ( isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
        ...
        $item_valid = crt_manage_validate_person($item);
        if ($item_valid === true) {
            if ($item['id'] == 0) {
    ...
    add_meta_box('persons_form_meta_box', 'Domain detail', 'crt_manage_persons_form_meta_box_handler', 'domain', 'normal', 'default');
 ...
 * @param $item
 */
function crt_manage_persons_form_meta_box_handler($item)
{
    ?>
 ...
 * @return bool|string
 */
function crt_manage_validate_person($item)
{
    $messages = array();
 ...
 * in this example plugin your_uniq_plugin_name == crt_manage
 *
